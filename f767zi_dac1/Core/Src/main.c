...
uint32_t callback_full = 0;
uint32_t callback_half = 0;
float angle = 0;
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	...
	while (1) {
		now = uwTick;
		if (now >= next_tick) {
			printf("Tick %lu (loop = %lu). callback=%lu, half=%lu, full=%lu \r\n", now / 1000, loop_counter, callback_counter, callback_half, callback_full);
			loop_counter = 0;
			next_tick = now + 1000;
		}
		++loop_counter;
